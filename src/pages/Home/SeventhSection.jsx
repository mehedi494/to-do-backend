      ...
      <div className="max-w-7xl mx-auto  font-baiJamjuree">
        ...
        <div className=" grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-2  gap-4 justify-items-center items-center">
          ...
          {cardContent?.map((card, index) => (
            <div key={index} className="flex flex-col gap-y-2 max-w-[350px] max-h-[600px] bg-white border shadow-md p-4 rounded-xl">
              <h1 className="text-center text-2xl font-semibold">
                {card?.catagories}
              </h1>
              <img className="rounded" src={card?.img} alt="rpgs" />
             {
              card?.list.map((item,index)=>( <p className="text-cente P-4 text-xl font-semibold text-[#feab2d]"> {item}</p>))
             }
